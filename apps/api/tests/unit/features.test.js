...
describe('isEnabled', () => {
  ...
  it('returns false when flag does not exist', async () => {
    FeatureFlag.findOne.mockResolvedValue(null);
    const result = await isEnabled('nonexistent');
    expect(result).toBe(false);
  });

  it('returns true when flag is enabled', async () => {
    FeatureFlag.findOne.mockResolvedValue({ enabled: true });
...
describe('setEnabled', () => {
  ...
  it('updates flag and invalidates cache', async () => {
    FeatureFlag.findOne.mockResolvedValue({ name: 'flag', enabled: false });
