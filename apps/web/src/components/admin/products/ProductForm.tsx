  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-h-[65vh] overflow-y-auto pr-1">
      <div className="grid grid-cols-2 gap-3">
        <div><label style={adminLabel}>Name</label><input required style={adminInput} value={form.name} onChange={(e) => set('name', e.target.value)} /></div>
        <div><label style={adminLabel}>SKU</label><input required style={adminInput} value={form.sku} onChange={(e) => set('sku', e.target.value)} /></div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div><label style={adminLabel}>Price (minor units)</label><input required type="number" min={0} style={adminInput} value={form.price} onChange={(e) => set('price', e.target.value)} /></div>
        <div><label style={adminLabel}>Stock</label><input required type="number" min={0} style={adminInput} value={form.stockOnHand} onChange={(e) => set('stockOnHand', e.target.value)} /></div>
      </div>
      <div>
      ...
      <AdminImageUploader label="Hero Image" value={form.image0} onChange={(v) => set('image0', v)} />
