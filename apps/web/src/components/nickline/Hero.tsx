...
export default function Hero({ onScrollToSection, heroImage }: HeroProps) {
  ...
  const currentSlide = {
    ...
    buttonText: 'SHOP NOW',
    linkTo: 'shop',
    image: heroImage || '/images/hero-product.jpg',
  };

  const handleShopNow = () => {
    if (onScrollToSection) {
      onScrollToSection(currentSlide.linkTo || 'shop');
    } else {
      router.push('/shop');
    }
  };
      ...
      <div className="relative z-10 w-full max-w-[1280px] mx-auto px-6 md:px-10 pt-24 pb-16">
        <div className="flex flex-col md:flex-row items-center gap-8 md:gap-4">
          ...
          <div className="w-full md:w-[55%] md:pr-8">
            ...
            <h1 className="text-display text-warm-white mb-6" aria-label="WEAR YOUR NECKLINE">
              {['WEAR', 'YOUR', 'NECKLINE'].map((word, wi) => (
            ...
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4, delay: 1.4, ease: easeOutExpo }}
          ...
          <div className="w-full md:w-[45%] flex items-center justify-center relative">
            ...
            >
              <img
                ...
                alt="NECKLINE Solid Perfume - Black tin on crimson velvet"
