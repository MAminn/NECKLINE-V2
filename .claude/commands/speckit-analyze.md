...
Check if `.specify/extensions.yml` exists and look for `hooks.before_analyze`. Process hooks per the standard hook logic (optional hooks prompt user; mandatory hooks execute before proceeding). Skip silently if no hooks or file missing.
...
After reporting, check if `.specify/extensions.yml` exists and look for `hooks.after_analyze`. Process hooks per the standard hook logic. Skip silently if no hooks or file missing.
