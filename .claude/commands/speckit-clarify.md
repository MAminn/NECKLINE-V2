...
Check if `.specify/extensions.yml` exists and look for `hooks.before_clarify`. Process hooks per the standard hook logic (optional hooks prompt user; mandatory hooks execute before proceeding). Skip silently if no hooks or file missing.
...
5. Integration after EACH accepted answer (incremental update approach):
    ...
    - Then immediately apply the clarification to the most appropriate section(s):
       ...
       - Data shape / entities → Update Data Model preserving ordering.
...
Check if `.specify/extensions.yml` exists and look for `hooks.after_clarify`. Process hooks per the standard hook logic. Skip silently if no hooks or file missing.
