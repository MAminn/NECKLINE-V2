...
- **FR-003**: System MUST collect and validate: full name, email address, phone number, shipping address (street, city, governorate/state, postal code, country), and optional order notes.
- **FR-004**: System MUST compute shipping cost server-side based on shipping address and selected shipping method; the customer must not be able to set or modify shipping cost.
...
- **FR-012**: System MUST release any active cart reservations upon successful order creation (converting them to committed stock) or upon checkout abandonment.
...
- **FR-016**: System MUST record audit events for every order lifecycle event: created, paid, shipped, delivered, cancelled.
...
- **Order**: Represents a completed purchase. Contains order number, status (pending, confirmed, processing, shipped, delivered, cancelled), customer info (name, email, phone), shipping address, shipping method, order total, currency, payment status, payment transaction reference, user ID (nullable for guest), line items snapshot, timestamps, audit trail.
...
- The initial shipping cost is a flat rate per method; dynamic weight-based or zone-based shipping is deferred.
